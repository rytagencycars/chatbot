...
(function(){
  function init(){
    ...
    function home(){
      var b = g('ryt-body');
      if(isVDP) b.innerHTML = '<button class="ryt-btn" onclick="window._rytForm(&quot;Si, me interesa!&quot;)">Si, me interesa!</button>';
    ...
    window._rytForm = function(intent){
      ...
      g('ryt-body').innerHTML = '<div class="ryt-arow">'+avMini+'<div class="ryt-amsg">'+AGENT_MSG+'</div></div><input type="text" id="ryt-n" class="ryt-inp" placeholder="Tu nombre*"><input type="tel" id="ryt-t" class="ryt-inp" placeholder="Tu telefono*"><button id="ryt-sub" class="ryt-send" onclick="window._rytSend()">ENVIAR SOLICITUD</button><button class="ryt-bkf" onclick="window._rytBk()">Volver</button>';
    };

    window._rytSend = function(){
      var n=g('ryt-n').value, ph=g('ryt-t').value;
      if(!n||!ph){ alert('Por favor, rellena nombre y telefono'); return; }
      ...
      xhr.send(JSON.stringify({firstName:n+" ["+lbl+"]",phone:ph,source:"CHATBOT WEB",tags:["chatbot"],notes:"Pagina: "+s.title+" | URL: "+s.url}));
    };
