/* CONFIG:{"nombre":"Gsancho","color":"#FFCC00","color2":"#1a1a1a","wa":"34608549040","waLabel":"¿Hablámos?","waMsg":"Hola, me gustaría recibir más información.","agentMsg":"Genial, completa tus datos y un comercial de nuestro equipo se pondrá en contacto contigo muy pronto.","vdp":"/main/","vdpUrl":"https://www.gsancho.es/main/producto/renault-megane-intens-tce-85-kw-115cv-gpf-ss/","webhook":"https://services.leadconnectorhq.com/hooks/oq7X7DQ0PSmkExuEI46j/webhook-trigger/5d6acf42-6a61-49b4-9f12-caf70dd841b8","avatar":"https://assets.cdn.filesafe.space/oq7X7DQ0PSmkExuEI46j/media/39d5f1a3-4882-468f-9fdd-8d149c4858c2.png","btns":["Quiero más información🚗"],"slug":"gsanchov2","updatedAt":"2026-05-11T21:12:12.642Z"} */
/* RYT AGENCY — Chatbot Web v3 */
(function(){
  function init(){
    if(window._rytBot)return;
    window._rytBot=true;
    var N='Gsancho',COLOR='#FFCC00',COLOR2='#1a1a1a',WA='34608549040';
    var WA_LABEL='¿Hablámos?',WA_MSG='Hola, me gustaría recibir más información.';
    var AGENT_MSG='Genial, completa tus datos y un comercial de nuestro equipo se pondrá en contacto contigo muy pronto.';
    var VDP='/main/';
    var BTNS=['Quiero más información🚗'];
    var AVATAR='https://assets.cdn.filesafe.space/oq7X7DQ0PSmkExuEI46j/media/39d5f1a3-4882-468f-9fdd-8d149c4858c2.png';
    ...
    st.innerHTML=`
      ...
      .ryt-trig{width:58px;height:58px;background:COLOR2_PH;border-radius:50%;border:2.5px solid COLOR_PH;box-sizing:border-box;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 6px 20px rgba(0,0,0,.2);}
      ...
      .ryt-hdr{padding:30px 14px 14px;text-align:center;position:relative;border-radius:18px 18px 0 0;}
      .ryt-hdr h3{margin:4px 0 0;font-size:15px;font-weight:800;color:COLOR_PH;}
      .ryt-hdr p{margin:4px 0 0;font-size:12px;color:CONTRAST_PH;opacity:.95;line-height:1.3;}
      .ryt-x{position:absolute;top:13px;right:14px;cursor:pointer;font-size:22px;color:CONTRAST_PH;opacity:.7;}
      .ryt-bk{position:absolute;top:13px;left:18px;cursor:pointer;font-size:20px;color:CONTRAST_PH;display:none;}
      ...
      .ryt-btn{width:100%;padding:11px;margin-bottom:8px;border:none;border-radius:10px;background:COLOR_PH;color:CONTRAST_BTN_PH;cursor:pointer;font-size:14px;font-weight:700;font-family:-apple-system,sans-serif;}
      ...
      .ryt-send{width:100%;padding:11px;border:none;border-radius:10px;background:COLOR_PH;color:CONTRAST_BTN_PH;cursor:pointer;font-size:14px;font-weight:700;margin-bottom:6px;font-family:-apple-system,sans-serif;}
    ...
    `.replace(/COLOR_PH/g,COLOR).replace(/COLOR2_PH/g,COLOR2).replace(/CONTRAST_PH/g,(function(hex){hex=hex.replace('#','');if(hex.length===3)hex=hex[0]+hex[0]+hex[1]+hex[1]+hex[2]+hex[2];var r=parseInt(hex.substr(0,2),16),g=parseInt(hex.substr(2,2),16),b=parseInt(hex.substr(4,2),16);return(0.299*r+0.587*g+0.114*b)/255>0.5?'#111111':'#ffffff';})(COLOR2));
  codeJS=codeJS.replace(/CONTRAST_BTN_PH/g,(function(hex){hex=hex.replace('#','');if(hex.length===3)hex=hex[0]+hex[0]+hex[1]+hex[1]+hex[2]+hex[2];var r=parseInt(hex.substr(0,2),16),g=parseInt(hex.substr(2,2),16),b=parseInt(hex.substr(4,2),16);return(0.299*r+0.587*g+0.114*b)/255>0.5?'#111111':'#ffffff';})(COLOR));
...
Web: '+s.url),'_blank');},1200);}
    var xhr=new XMLHttpRequest();xhr.open("POST","https://services.leadconnectorhq.com/hooks/oq7X7DQ0PSmkExuEI46j/webhook-trigger/5d6acf42-6a61-49b4-9f12-caf70dd841b8",true);xhr.setRequestHeader("Content-Type","application/json");xhr.onreadystatechange=function(){if(xhr.readyState===4)done(n,ph,s);};var lbl=s.isVDP?"VDP: "+s.title:s.intent;xhr.send(JSON.stringify({firstName:n+" ["+lbl+"]",phone:ph,source:"CHATBOT WEB",tags:["chatbot"],notes:"Página: "+s.title}));};
    home();
