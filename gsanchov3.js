 ...
 * Configuración para despliegue externo (GitHub / CDN)
 */
...
(function(){
  function init(){
    ...
    var N = 'Gsancho',
        ...
        WA = '34608549040',
        WA_LABEL = '¿Hablámos?',
        WA_MSG = 'Hola, me gustaría recibir más información.',
        AGENT_MSG = 'Genial, completa tus datos y un comercial de nuestro equipo se pondrá en contacto contigo muy pronto.',
        VDP_PATH = '/main/',
    ...
    // --- CÁLCULO DE CONTRASTES AUTOMÁTICO ---
    ...
    st.innerHTML = `
      ...
      .ryt-bkf{width:100%;padding:11px;border:.5 solid #ddd;border-radius:13px;background:#f5f5f5;color:#555;cursor:pointer;font-size:13px;font-family:-apple-system,sans-serif;}
    ...
    var avMini = AVATAR ? '<img src="' + AVATAR + '" style="width:28px;height:28px;border-radius:50%;object-fit:cover;flex-shrink:0;">' : '';
    
    wrap.innerHTML = `
      <div id="ryt-card" class="ryt-card">
        <div class="ryt-hdr">
          ${avHTML}
          <span id="ryt-bk" class="ryt-bk" onclick="window._rytBk()">←</span>
          <span class="ryt-x" onclick="window._rytTog()">×</span>
          <h3>${N}</h3>
          <p id="ryt-msg">${wMsg}</p>
        </div>
        <div id="ryt-body" class="ryt-body"></div>
        <div class="ryt-foot">Desarrollado por <a href="https://rytagency.es/" target="_blank">RYT AGENCY</a></div>
      </div>
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:5px;">
        <div class="ryt-badge">En línea <span class="ryt-dot"></span></div>
        <div class="ryt-trig" onclick="window._rytTog()">
    ...
    function home() {
      var b = g('ryt-body');
      if(isVDP) b.innerHTML = '<button class="ryt-btn" onclick="window._rytForm(\'¡Sí, me interesa! ✅\')">¡Sí, me interesa! ✅</button>';
      else b.innerHTML = BTNS.map(function(btn) { return '<button class="ryt-btn" onclick="window._rytForm(\'' + btn + '\')">' + btn + '</button>'; }).join('');
    }

    window._rytForm = function(intent) {
      s.intent = intent;
      g('ryt-bk').style.display = 'block';
      g('ryt-body').innerHTML = `
        <div class="ryt-arow">${avMini}<div class="ryt-amsg">${AGENT_MSG}</div></div>
        <input type="text" id="ryt-n" class="ryt-inp" placeholder="Tu nombre*">
        <input type="tel" id="ryt-t" class="ryt-inp" placeholder="Tu teléfono*">
        <button id="ryt-sub" class="ryt-send" onclick="window._rytSend()">ENVIAR SOLICITUD</button>
        <button class="ryt-bkf" onclick="window._rytBk()">Volver</button>`;
    };

    window._rytSend = function() {
      var n = g('ryt-n').value, ph = g('ryt-t').value;
      if(!n || !ph) { alert('Por favor, rellena nombre y teléfono'); return; }
      ...
      // Envío al Webhook de GoHighLevel / LeadConnector
      var xhr = new XMLHttpRequest();
      xhr.open("POST", WEBHOOK, true);
      xhr.setRequestHeader("Content-Type", "application/json");
      xhr.onreadystatechange = function() { if(xhr.readyState === 4) done(n, ph, s); };
      var lbl = s.isVDP ? "VDP: " + s.title : s.intent;
      xhr.send(JSON.stringify({
        firstName: n + " [" + lbl + "]",
        phone: ph,
        source: "CHATBOT WEB",
        tags: ["chatbot"],
        notes: "Página: " + s.title + " | URL: " + s.url
      }));
    };

    home();
    // Apertura automática a los 2.5 segundos si no se ha abierto manualmente
